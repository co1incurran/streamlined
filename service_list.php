...
<tbody>
	...
	?>
			<tr>	
				...
				<td><?php echo ucwords($results['model'])?></td>
				<td><?php echo $results['serialid']?></td>
				<td><?php echo $results['installation_date']?></td>
				<td><?php echo $results['service_date'];
					$lastService = date($results['service_date']);
					//echo $lastService;
				?></td>
				<td><?php 
				//this checks if the service is overdue 
					$lastService=date('Y-m-d', strtotime('+1 year', strtotime($lastService)) );
					//echo $lastService.' ' ;
					if($lastService > $date){
						if($results['next_service'] == '0000-00-00'){
							echo $lastService;
						}else{
							echo $results['next_service'];
						}
					}else{
						echo 'Overdue';
					}
